...
std::string GenerateKey(uint64_t cuid, int row_id) {
  auto encode_u64_be = [](uint64_t v, char* out) {
    for (int i = 0; i < 8; ++i) {
  ...
  // Key layout: [dbid:8B][tableid:8B][cuid:8B][rowid:10B ascii][reserved]
  encode_u64_be(kDbId, &key[0]);
  encode_u64_be(kTableId, &key[8]);
  encode_u64_be(cuid, &key[16]);
  char row_buf[16];
...
class PerfRunner {
 public:
  ...
  void Run(int thread_id, std::atomic<bool>* stop, ThreadStats* stats) {
    ...
    // Temporary mitigation: bypass delta hot path to avoid missing rows during
    // high-concurrency validation while investigating root cause.
 ...
 private:
  bool DoPut(uint64_t cuid, int start_row, ThreadStats* stats) {
    ...
    if (!s.ok()) {
      std::cerr << "[ERROR] Put failed for CUID " << cuid
                << ", start_row=" << start_row
  ...
  int DoScan(uint64_t cuid, int cur_rows, bool full_scan, const rocksdb::ReadOptions& ro, ThreadStats* stats) {
    ...
    // Use regular iterator path for correctness validation in stress test.
    // The delta_full_scan fast path may skip data under heavy concurrency.
  ...
  void DoDelete(uint64_t cuid, int total_rows, ThreadStats* stats) {
    ...
    for (int i = 0; i < total_rows; ++i) {
      auto start = std::chrono::high_resolution_clock::now();
      rocksdb::Status s = db_->Delete(wo, GenerateKey(cuid, i));
      auto end = std::chrono::high_resolution_clock::now();
      if (!s.ok()) {
        std::cerr << "[WARN] Delete failed for CUID " << cuid
                  << ", row=" << i
                  << ", status=" << s.ToString() << std::endl;
        continue;
      }
